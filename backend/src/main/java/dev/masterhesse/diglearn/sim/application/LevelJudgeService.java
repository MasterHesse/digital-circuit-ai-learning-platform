...
import dev.masterhesse.diglearn.sim.api.LevelJudgeApiModels;
import dev.masterhesse.diglearn.sim.judge.CircuitSpec;
import dev.masterhesse.diglearn.sim.judge.CircuitValidator;
import dev.masterhesse.diglearn.sim.judge.IterativeBooleanJudgeEvaluator;
import dev.masterhesse.diglearn.sim.judge.SimpleBooleanJudgeEvaluator;
...
public class LevelJudgeService {
    ...
    private final IterativeBooleanJudgeEvaluator iterativeEvaluator = new IterativeBooleanJudgeEvaluator();
    ...
    public LevelJudgeApiModels.JudgeResponse judge(String levelCode, String userId, JsonNode circuitNode) {
        ...
        boolean allowCycles = level.isAllowCycles();
        ...
        try {
            CircuitValidator.validate(circuit, allowedComponents, allowCycles);
        ...
        for (LevelTestCaseEntity tc : testCases) {
            ...
            for (LevelTestStepEntity step : steps) {
                ...
                try {
                    ...
                    if (allowCycles) {
                        allOut = iterativeEvaluator.evaluateStep(circuit, inputs, state);
                        state = allOut; // 下一 step 的 prevState
                    } else {
                        allOut = evaluator.evaluateAll(circuit, inputs);
                    }
                } catch (IllegalArgumentException e) {
